...
if (drawTri)
{
	interactRange = lerp(interactRange, oPlayer.interactDistanceMax, 0.1);
	offset = lerp(offset, 22.5, 0.1);
	triAlpha = scrApproach(triAlpha, 0.1, 0.01);
...
{
	interactRange = lerp(interactRange, oPlayer.interactDistanceMax + scrWave(-2, 2, 2, 0), 0.1);
	offset = lerp(offset, 22.5 + scrWave(-4, 4, 6, 0), 0.1);
	triAlpha = scrApproach(triAlpha, 0, 0.01);
